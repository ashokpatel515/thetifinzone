					...
					if(element.is(':visible') && currentScrollTop > scrollMinPos && !element.hasClass('supsystic-sticky-active')) {	// Start sticking
						...
						if(element.hasClass('supsystic-sticky-active')) {
							if(footerHeight) {
								...
								if(topDiff < 0 && element.attr('id') != 'gmpMapRightStickyBar') {	// gmpMapRightStickyBar = id of google maps preview container (fix is needed for normal scroll of map preview container)
									element.css({
